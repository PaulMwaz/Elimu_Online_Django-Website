(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const a of r.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&n(a)}).observe(document,{childList:!0,subtree:!0});function o(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(s){if(s.ep)return;s.ep=!0;const r=o(s);fetch(s.href,r)}})();const p=window.location.hostname==="localhost"?"http://127.0.0.1:8000/api":"https://elimu-backend-59739536402.europe-west1.run.app/api";function L(e,t=null){console.log("🔐 Saving tokens to localStorage..."),localStorage.setItem("auth_token",e),t&&localStorage.setItem("refresh_token",t)}function k(){const e=localStorage.getItem("auth_token");return console.log("🔎 Fetched access token:",e?"✔️ Found":"❌ Missing"),e}function m(){const e=!!k();return console.log("👁️ Logged in:",e),e}function M(){console.log("🚪 Logging out: clearing localStorage tokens & user..."),localStorage.removeItem("auth_token"),localStorage.removeItem("refresh_token"),localStorage.removeItem("auth_user")}async function C(e,t){console.log("🚀 Attempting login with:",e);const o=await fetch(`${p}/token/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:e,password:t})});if(!o.ok){const r=await o.json().catch(()=>({message:"Login failed"}));throw console.error("❌ Login failed:",r),new Error(r.detail||r.message||"Invalid credentials")}const n=await o.json();console.log("✅ JWT tokens received:",n),L(n.access,n.refresh);const s=await F(n.access);return localStorage.setItem("auth_user",JSON.stringify(s)),console.log("👤 Logged-in user profile:",s),{token:n.access,user:s}}async function F(e){console.log("📡 Fetching user profile with token...");const t=await fetch(`${p}/users/me/`,{headers:{Authorization:`Bearer ${e}`}});return t.ok?await t.json():(console.warn("⚠️ Failed to fetch profile. Using fallback."),{name:"User",email:"unknown"})}function N(){const e=localStorage.getItem("auth_user");return console.log("🧾 Current user from localStorage:",e),e?JSON.parse(e):null}async function A(e){console.log("📝 Registering user with data:",e);const t=await fetch(`${p}/users/register/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!t.ok){const n=await t.json().catch(()=>({message:"Registration failed"}));throw console.error("❌ Registration error:",n),new Error(n.message||"Signup error")}const o=await t.json();return console.log("✅ Registration successful:",o),o}function $(){const e=["/images/slide1.jpg","/images/slide2.jpg","/images/slide3.jpg"];let t=0;const o=document.createElement("div");o.className="relative w-screen h-screen overflow-hidden";const n=document.createElement("img");n.src=e[t],n.alt="Hero Slide",n.className="absolute inset-0 w-full h-full object-cover transition-opacity duration-700",n.onerror=()=>{n.src="/images/placeholder.jpg"},o.appendChild(n);const s=document.createElement("div");s.className="absolute inset-0 bg-black bg-opacity-30 flex flex-col items-center justify-center text-white text-center px-2 sm:px-6",s.innerHTML=`
  ...
  `,o.appendChild(s);const r=document.createElement("button");r.innerHTML="←",r.className="absolute left-1 sm:left-3 top-1/2 -translate-y-1/2 transform bg-black bg-opacity-50 text-white rounded-full w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center hover:bg-opacity-80 z-20";const a=document.createElement("button");a.innerHTML="→",a.className="absolute right-1 sm:right-3 top-1/2 -translate-y-1/2 transform bg-black bg-opacity-50 text-white rounded-full w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center hover:bg-opacity-80 z-20",o.appendChild(r),o.appendChild(a);function d(){n.src=e[t]}return r.addEventListener("click",()=>{t=(t-1+e.length)%e.length,d()}),a.addEventListener("click",()=>{t=(t+1)%e.length,d()}),setInterval(()=>{t=(t+1)%e.length,d()},6e3),o}function P(){const e=document.createElement("section");return e.className="bg-[#F4FFF6] py-16 px-6",e.innerHTML=`
  ...
  `,e.appendChild(t);const o=t.querySelector("#resource-grid"),n=[{image:"/images/sample1.jpg",title:"KCSE Chemistry Paper 1 - Nov 2017",subject:"Chemistry",form:"Form 4",category:"Exams",author:"KNEC",price:0,isFree:!0},{image:"/images/sample2.jpg",title:"Form 1 Mid Term Past Paper - English & Literature",subject:"English",form:"Form 1",category:"Exams",author:"ELIMU",price:0,isFree:!0},{image:"/images/sample3.jpg",title:"Form 4 Mathematics Paper 2 - Mock Exam (Sept 2019)",subject:"Maths",form:"Form 4",category:"Exams",author:"Busy Teacher",price:125,isFree:!1}],s=a=>{o.innerHTML=a.map(R).join("")},r=()=>{const a=document.getElementById("search-input").value.toLowerCase(),d=document.getElementById("filter-category").value,i=document.getElementById("filter-form").value,l=n.filter(c=>c.title.toLowerCase().includes(a)&&(d?c.category===d:!0)&&(i?c.form===i:!0));s(l)};return setTimeout(()=>{document.getElementById("search-input").addEventListener("input",r),document.getElementById("filter-category").addEventListener("change",r),document.getElementById("filter-form").addEventListener("change",r),s(n)},100),e}function R({image:e,title:t,subject:o,form:n,category:s,author:r,price:a,isFree:d}){const i=d?'<span class="text-green-600 font-semibold text-sm">Free</span>':`<span class="text-yellow-600 font-semibold text-sm">KSh ${a}.00</span>`;return`
    <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition duration-300 flex flex-col">
      ...
      <div class="p-4 flex-grow flex flex-col justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500">${o} · ${n}</p>
  ...
  `}function j(){const e=document.createElement("section");e.className="bg-gray-50 pt-[80px]";const t=$(),o=P(),n=B(),s=T();return e.appendChild(t),e.appendChild(o),e.appendChild(n),e.appendChild(s),e}function q(){const e=document.createElement("section");return e.className="min-h-screen bg-gray-100 p-10 text-center",e.innerHTML=`
  ...
  `,t.querySelectorAll("a").forEach(n=>{n.addEventListener("click",s=>{s.preventDefault();const r=n.getAttribute("href");console.log("🔗 SPA link clicked:",r),window.history.pushState({},"",r),window.dispatchEvent(new Event("popstate"))})}),t.querySelector("form").addEventListener("submit",async n=>{n.preventDefault();const s=t.querySelector("#name").value.trim(),r=t.querySelector("#email").value.trim(),a=t.querySelector("#password").value.trim();console.log("📝 Attempting registration with:",{name:s,email:r});try{const d=await A({name:s,email:r,password:a});console.log("✅ Registration response:",d),alert("🎉 Registration successful. Please login."),window.history.pushState({},"","/login"),window.dispatchEvent(new Event("popstate"))}catch(d){console.error("❌ Registration error:",d.message),alert(`Error: ${d.message}`)}}),e.appendChild(t),e}function O(){const e=document.createElement("section");e.className="min-h-screen bg-gray-100 flex items-center justify-center px-4";const t=document.createElement("div");return t.className="w-full max-w-md bg-white p-8 rounded shadow-md",t.innerHTML=`
  ...
  `,t.querySelectorAll("a").forEach(n=>{n.addEventListener("click",s=>{s.preventDefault();const r=n.getAttribute("href");console.log("🔗 SPA navigation to:",r),window.history.pushState({},"",r),window.dispatchEvent(new Event("popstate"))})}),t.querySelector("form").addEventListener("submit",async n=>{n.preventDefault();const s=t.querySelector("#email").value.trim(),r=t.querySelector("#password").value.trim();console.log("🔐 Attempting login with:",{email:s,password:r});try{const a=await C(s,r);console.log("✅ Login success:",a);const d=document.querySelector("nav");d?d.replaceWith(v()):console.warn("⚠️ Navbar not found to refresh."),alert("Login successful!"),window.history.pushState({},"","/dashboard"),window.dispatchEvent(new Event("popstate"))}catch(a){console.error("❌ Login failed:",a.message),alert(`Error: ${a.message}`)}}),e.appendChild(t),e}const x=window.location.hostname==="localhost"?"http://127.0.0.1:8000/api":"https://elimu-backend-59739536402.europe-west1.run.app/api";console.log("🌍 API Base URL:",x);async function H(e,t={}){console.log("📡 [fetchJSON] Requesting:",e),console.log("📨 [fetchJSON] Options:",t);const o=await fetch(e,{headers:{"Content-Type":"application/json",...t.headers},credentials:"include",...t});if(!o.ok){const s=await o.json().catch(()=>({message:o.statusText}));throw console.error("❌ [fetchJSON] API Error:",o.status,s.message),new Error(s.message||"Request failed")}const n=await o.json();return console.log("✅ [fetchJSON] Response OK:",n),n}async function U(){return console.log("📥 [fetchResources] Fetching resources..."),await H(`${x}/resources/`)}function D(e,t){console.log("📦 Opening Preview Modal for:",t,e);const o=document.createElement("div");o.className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50",o.innerHTML=`
  ...
  `,o.querySelector("#cancel-payment-btn").addEventListener("click",()=>{console.log("🚫 Payment Modal closed by user."),o.remove()}),o.querySelector("#pay-now-btn").addEventListener("click",()=>{const n=o.querySelector("#mpesa-phone").value.trim();if(!/^07\d{8}$/.test(n)){alert("❌ Enter a valid Safaricom number (e.g., 0712345678)");return}console.log("📲 Initiating STK Push to:",n,"for resource:",t),alert(`M-Pesa STK Push initiated to ${n} for "${e}"`),o.remove()}),document.body.appendChild(o),console.log("✅ Payment Modal mounted.")}function W(){console.log("🟪 DEBUG: Rendering Dashboard");const e=document.createElement("section");return e.className="flex min-h-screen",e.innerHTML=`
  ...
  `,e.querySelectorAll(".btn-resource").forEach(t=>{t.addEventListener("click",()=>{const o=t.getAttribute("data-type");console.log(`📁 DEBUG: Resource type selected → ${o}`),K(o)})}),e}async function K(e){const t=document.getElementById("dashboard-content"),o={notes:"Notes",ebooks:"E-Books",exams:"Exams",schemes:"Schemes of Work",lessons:"Lesson Plans"},n=e==="notes"||e==="ebooks"?"level":"term";try{console.log("📦 DEBUG: Fetching resources from API...");const r=(await U()).filter(i=>i.category===o[e]);console.log(`📦 DEBUG: Total fetched → ${r.length}`),console.table(r);const a={};r.forEach(i=>{const l=i[n]||"Uncategorized";a[l]||(a[l]=[]),a[l].push(i)});let d=`<h2 class="text-2xl font-bold text-[#5624d0] mb-4">${o[e]}</h2>`;for(const[i,l]of Object.entries(a))console.log(`📂 DEBUG: Rendering group → ${i} (${l.length})`),d+=`
        <div class="mb-6">
      ...
      `;t.innerHTML=d,document.querySelectorAll(".btn-view").forEach(i=>{i.addEventListener("click",()=>{const l=i.getAttribute("data-url"),c=i.getAttribute("data-title");console.log(`👁️ DEBUG: Preview clicked → ${c}`),D(l,c)})}),document.querySelectorAll(".btn-pay").forEach(i=>{i.addEventListener("click",()=>{const l=i.getAttribute("data-id"),c=i.getAttribute("data-title"),u=i.getAttribute("data-price");console.log(`💳 DEBUG: Unlocking premium → ${c} | ID: ${l} | Price: ${u}`),_(c,l)})})}catch(s){console.error("❌ Failed to fetch or render resources:",s),t.innerHTML='<p class="text-red-500">Failed to load resources. Please try again later.</p>'}}function b(){const e=document.getElementById("page-content");if(!e){console.warn("⚠️ router(): #page-content container not found");return}e.innerHTML="";const t=window.location.pathname;console.log(`🔀 router(): Navigating to → ${t}`),t==="/"?(console.log("📄 Rendering → Home"),e.appendChild(j())):t==="/about"?(console.log("📄 Rendering → About"),e.appendChild(q())):t==="/signup"?(console.log("📄 Rendering → Signup"),e.appendChild(I())):t==="/login"?(console.log("📄 Rendering → Login"),e.appendChild(O())):["/dashboard","/notes","/ebooks","/exams","/schemes","/lessons"].includes(t)?(console.log(`📄 Rendering Dashboard for → ${t}`),e.appendChild(W())):(console.error("❌ 404 - Unknown route:",t),e.innerHTML=`
    ...
    `)}function v(){const e=N();console.log("👤 Current User in Navbar:",e);const t=document.createElement("nav");t.className="fixed top-0 left-0 w-full h-[64px] bg-white shadow z-50 flex items-center justify-between px-6",t.innerHTML=`
  ...
  `,t.querySelectorAll("a").forEach(l=>{l.addEventListener("click",y)});const o=t.querySelector("#menuToggle"),n=t.querySelector("#menuIcon"),s=t.querySelector("#mobileMenu");o==null||o.addEventListener("click",()=>{const l=!s.classList.contains("hidden");console.log("📱 Menu toggle clicked. Open:",!l),s.classList.toggle("hidden"),n.textContent=l?"☰":"✖"});const r=t.querySelector("#dropdownToggle"),a=t.querySelector("#dropdownMenu");r==null||r.addEventListener("click",()=>{a.classList.toggle("hidden")}),document.addEventListener("click",l=>{t.contains(l.target)||a==null||a.classList.add("hidden")});const d=t.querySelector("#logoutBtn"),i=t.querySelector("#logoutBtnMobile");return[d,i].forEach(l=>{l&&l.addEventListener("click",()=>{console.log("🚪 Logging out user..."),M();const c=t.querySelector("#auth-section"),u=t.querySelector("#mobile-auth-links");c&&(c.innerHTML=g(),f(c)),u&&(u.innerHTML=h(),f(u)),s==null||s.classList.add("hidden"),n.textContent="☰",window.history.pushState({},"","/"),b()})}),t}function y(e){e.preventDefault();const t=e.target.getAttribute("href");console.log("🔗 Navigating to:",t),window.history.pushState({},"",t),window.dispatchEvent(new Event("popstate"))}function g(){return console.log("🔐 Rendering logged-out auth links"),`
  ...
  `}function f(e){e.querySelectorAll("a").forEach(t=>{t.addEventListener("click",y)})}function G(){const e=document.createElement("footer"),t=window.location.pathname;return console.log("📍 Rendering footer for path:",t),t.startsWith("/dashboard")||t.startsWith("/notes")||t.startsWith("/exams")||t.startsWith("/ebooks")?(console.log("🧾 Using minimal footer layout for internal pages"),e.className="bg-gray-900 text-gray-400 text-center py-4 text-sm",e.innerHTML=`
    ...
    `),e}function w(){const e=document.getElementById("app");e.innerHTML="";const t=document.createElement("div");t.className="min-h-screen flex flex-col bg-gray-50";const o=v();t.appendChild(o);const n=window.location.pathname==="/",s=document.createElement("main");s.id="page-content",s.className=n?"flex-grow":"flex-grow mt-[80px] px-4 sm:px-6 md:px-8 py-6",t.appendChild(s);const r=G();t.appendChild(r),e.appendChild(t),b()}window.addEventListener("DOMContentLoaded",()=>{console.log("🚀 App Loaded (DOMContentLoaded)"),window.scrollTo(0,0),w()});window.addEventListener("popstate",()=>{console.log("🔙 SPA Navigation (popstate)"),window.scrollTo(0,0),w()});
